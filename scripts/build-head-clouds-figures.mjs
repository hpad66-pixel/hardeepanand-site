...
function cloudsGround(mobile = false) {
  ...
  let b = '';
  if (mobile) {
    b += reveal(`<path d="M75 86 C50 56 91 30 126 50 C145 14 205 26 208 67 C250 58 282 96 258 126 H80 C48 126 48 98 75 86 Z" fill="${C.blue}" fill-opacity="0.13" stroke="${C.blue}" stroke-width="2.4"/>${text(168, 82, 'Vision', { size: 32, color: C.blue, serif: true, weight: 500 })}${text(168, 112, 'what the system could become', { size: 13, color: C.muted })}`, 0);
    ...
    cards.forEach(([head, sub], i) => {
      ...
      b += reveal(box(48, y, 284, 70, { fill: 'var(--surface,#f5f3ed)' }) + text(190, y + 30, head, { size: 20, weight: 700 }) + text(190, y + 54, sub, { size: 13, color: C.muted }), 1.25 + i * .25);
    });
  } else {
    b += `<path d="M0 330 C112 300 212 340 322 318 C462 292 553 352 690 314 C760 294 790 304 820 292 L820 410 L0 410 Z" fill="${C.blue}" fill-opacity="0.08"/>`;
    b += reveal(`<path d="M88 110 C52 70 110 35 158 62 C183 12 264 29 266 86 C326 76 365 130 329 168 H92 C50 168 50 124 88 110 Z" fill="${C.blue}" fill-opacity="0.13" stroke="${C.blue}" stroke-width="2.5"/>${text(208, 102, 'Vision', { size: 36, color: C.blue, serif: true, weight: 500 })}${text(208, 136, 'what the system could become', { size: 14, color: C.muted })}`, 0);
    b += line('M335 136 C390 138 430 130 492 128', { arrow: `${id}-arrow`, delay: .35 });
    b += reveal(box(480, 64, 262, 160, { fill: C.tint, stroke: C.blue, rx: 18 }) + text(611, 102, ['Ground test', 'evidence', 'people', 'authority'], { size: 26, color: C.ink, serif: true, gap: 1.05 }) + text(611, 204, 'not just a speech', { size: 13, color: C.muted }), .55);
    ...
    cards.forEach(([x, y, head, sub], i) => {
      b += reveal(box(x, y, 178, 86, { fill: 'var(--surface,#f5f3ed)' }) + text(x + 89, y + 36, head, { size: 21, weight: 700 }) + text(x + 89, y + 64, sub, { size: 13, color: C.muted }), 1.05 + i * .25);
...
function proofTest(mobile = false) {
  ...
  if (mobile) {
    const stages = [
      [46, 28, 288, 76, C.tint, C.blue, ['Disruptive move', 'pressure enters the room'], C.ink],
      [46, 148, 288, 86, 'var(--surface,#f5f3ed)', C.blue, ['Burden of proof', 'what improves, who owns it'], C.ink],
      [46, 282, 288, 104, C.paper, C.line, ['If it cannot answer', 'noise, heat, reaction'], C.ink],
      [46, 434, 288, 128, 'var(--surface,#f5f3ed)', C.blue, ['If it can answer', 'clearer decision · visible risk', 'better human work'], C.ink],
    ];
    stages.forEach(([x, y, bw, bh, fill, stroke, rows, color], i) => {
      b += reveal(box(x, y, bw, bh, { fill, stroke, rx: 16 }) + text(190, y + 34, rows[0], { size: 21, color, weight: 700 }) + text(190, y + 61, rows.slice(1), { size: 14, color: C.muted }), i * .55);
      if (i < stages.length - 1) b += line(`M190 ${y + bh + 4} V${stages[i + 1][1] - 6}`, { arrow: `${id}-arrow`, delay: .25 + i * .55 });
    });
  } else {
    b += reveal(box(286, 36, 248, 80, { fill: C.tint, stroke: C.blue, rx: 18 }) + text(410, 69, 'Disruptive move', { size: 22, color: C.ink, weight: 700 }) + text(410, 96, 'pressure enters the room', { size: 14, color: C.muted }), 0);
    b += line('M410 118 V158', { arrow: `${id}-arrow`, delay: .35 });
    b += reveal(box(226, 156, 368, 92, { fill: 'var(--surface,#f5f3ed)', stroke: C.blue, rx: 17 }) + text(410, 194, ['Can it carry', 'a burden of proof?'], { size: 23, weight: 700, gap: 1.08 }), .7);
    ...
    b += reveal(box(592, 270, 192, 112, { fill: 'var(--surface,#f5f3ed)', stroke: C.blue }) + text(688, 312, 'Yes', { size: 24, weight: 700 }) + text(688, 342, ['evidence, ownership', 'and follow-through'], { size: 14, color: C.muted }), 1.7);
...
function utilityPath(mobile = false) {
  ...
  if (mobile) {
    ...
    steps.forEach(([head, sub], i) => {
      const y = 34 + i * 150;
      const emphasis = i === 3;
      b += reveal(box(42, y, 296, i === 3 ? 108 : 96, { fill: emphasis || i === 1 ? C.tint : C.paper, stroke: emphasis || i === 1 ? C.blue : C.line, rx: 17 }) + text(190, y + 41, head, { size: 22, color: C.ink, weight: 700 }) + text(190, y + 70, sub, { size: 14, color: C.muted }), i * .55);
    ...
    b += reveal(box(318, 70, 224, 118, { fill: 'var(--surface,#f5f3ed)', stroke: C.blue, rx: 18 }) + text(430, 112, 'Source packet', { size: 24, weight: 700 }) + text(430, 142, ['missing record', 'owner · risk', 'next action'], { size: 14, color: C.muted, gap: 1.2 }), 1.05);
    ...
    b += reveal(box(664, 148, 132, 168, { fill: C.tint, stroke: C.blue, rx: 20 }) + text(730, 204, ['Better', 'decision'], { size: 24, color: C.ink, serif: true, gap: 1.05 }) + text(730, 260, ['clear enough', 'to own'], { size: 13, color: C.muted }), 2.75);
  }
...
function adaptationLoop(mobile = false) {
  ...
  if (mobile) {
    ...
    steps.forEach(([head, sub], i) => {
      const y = 30 + i * 124;
      const emphasis = i === 4;
      b += reveal(box(48, y, 284, 82, { fill: emphasis ? C.tint : C.paper, stroke: emphasis ? C.blue : C.line, rx: 16 }) + text(190, y + 34, head, { size: 21, color: C.ink, weight: 700 }) + text(190, y + 60, sub, { size: 14, color: C.muted }), i * .45);
    ...
    b += pulse(`<circle cx="410" cy="216" r="92" fill="${C.blue}" fill-opacity="0.10" stroke="${C.blue}" stroke-width="2"/>${text(410, 208, 'Adaptation', { size: 25, weight: 700 })}${text(410, 236, 'practice with judgment', { size: 14, color: C.muted })}`, 1.15);
