...
export async function submitInquiry({request,env}, fetcher=fetch, kind='inquiry') {
  ...
  const name=clean(input.name,100),email=clean(input.email,254),company=clean(input.company||'',150),message=subscription?'Reader asked to be notified about new Systems Lens essays and related APAS ideas.':clean(input.message,4000),articlePath=clean(input.articlePath,200);
