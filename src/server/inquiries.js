...
const interestForArticle = (articlePath, subscription) => {
  if (subscription) return 'reader-connection';
  if (articlePath === '/writing/governance-at-operating-speed/' || articlePath === '/writing/governance-must-live-inside-the-work/') return 'governance-implementation';
  if (articlePath === '/climb/adapt-dont-pivot/') return 'academy-learning';
  if (articlePath === '/writing/the-question-missing-from-the-ai-jobs-debate/') return 'knowledge-access';
  if (articlePath === '/writing/vital-signs/') return 'measurement-readiness';
  if (articlePath.startsWith('/writing/')) return 'data-readiness';
  return 'advisory';
};
export async function submitInquiry({request,env}, fetcher=fetch, kind='inquiry') {
  const subscription=kind==='subscription';
  if(!configured(env)) return json(503,{error:'Online delivery is unavailable. Please use the email link.'});
  ...
  const name=clean(input.name,100),email=clean(input.email,254),company=clean(input.company||'',150),message=subscription?'Reader requested an APAS connection while continuing to Substack. Subscription confirmation is managed by Substack.':clean(input.message,4000),articlePath=clean(input.articlePath,200);
  if(!name||!email||!/^\S+@\S+\.\S+$/.test(email)||company===null||!message||input.consent!==true||!(subscription ? articlePath==='/' : /^\/(writing|climb)\/[a-z0-9-]+\/$/.test(articlePath||''))||!/^\w{8}-\w{4}-4\w{3}-[89ab]\w{3}-\w{12}$/i.test(input.id||'')) return json(400,{error:'Please provide your name, email, note, and permission to respond.'});
  const submission={id:input.id,name,email,company,message,articlePath,consent:true,...(subscription?{kind}: {})};
  ...
  let response;
  try{response=await fetcher(endpoint,{method:'POST',redirect:'manual',signal:AbortSignal.timeout(15000),headers:{'Content-Type':'application/json','Authorization':`Bearer ${env.APAS_CRM_API_KEY}`,'X-APAS-Organization-ID':env.APAS_ORGANIZATION_ID,'CF-Access-Client-Id':env.CF_ACCESS_CLIENT_ID,'CF-Access-Client-Secret':env.CF_ACCESS_CLIENT_SECRET},body:JSON.stringify({externalId:submission.id,source:'website',person:{displayName:name,email,...(company?{companyName:company}:{})},context:{siteId:env.APAS_SITE_ID,...(env.APAS_PROJECT_ID?{projectId:env.APAS_PROJECT_ID}:{}),campaign:articlePath},consent:{recorded:true,reference:(subscription?'Website signup: permission for APAS to store contact details and connect about these ideas; Substack subscription not confirmed; ':'Website inquiry: permission for APAS to store this note and respond; ')+record.createdAt},metadata:{message,form:subscription?'systems-lens-reader':'systems-lens-article',...(subscription?{subscriptionStatus:'unconfirmed',subscriptionProvider:'substack'}:{}),interest:interestForArticle(articlePath,subscription),articleUrl:`https://hardeepanand.com${articlePath}`}})});}catch{return json(502,{error:'Your note is saved, but delivery is delayed. Please retry or contact us by email.'});}
  const result=await response.json().catch(()=>({}));
  if(response.status!==202||!result.data?.candidateId)return json(502,{error:'Your note is saved, but delivery is not confirmed. Please retry or contact us by email.'});
...
export const submitSubscription = (context, fetcher=fetch) => submitInquiry(context,fetcher,'subscription');
