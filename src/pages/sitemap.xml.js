...
export function GET(){
 const paths=['/','/about/','/writing/','/climb/','/case-studies/','/ai-watch/','/summits/','/work/',...publicPages.map(p=>`/${p.path}/`),...readArticles(['substack/2026','linkedin/2026','owos/2026','author/2026']).filter(p=>p.status==='PUBLISHED').map(p=>`/writing/${p.slug}/`)];
