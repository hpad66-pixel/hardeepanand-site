...
export function addTakeawayActions(html, title, canonical, publication = 'The Systems Lens') {
  return html.replace(/<section class="article-takeaways"[\s\S]*?<\/section>/, section => {
    ...
    const attribution = `<p class="takeaways-attribution">${title}<br>Hardeep Anand · ${publication}<br>${canonical}</p>`;
